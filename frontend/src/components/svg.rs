...
use yew::{function_component, Callback, html, AttrValue, Html, Properties};
...
pub fn instagram(
    SvgProps { class, alt, color, width, height, onclick }: &SvgProps,
) -> Html {
    let children = html!{<><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect>
    <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path>
    <line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></>};

    html!{<SvgWrapper {children} {class} {alt} {onclick} fill={None} stroke={Some(color.clone())} {width} {height}/>}
}
...
fn svg_wrapper(
    ...
    html! {<svg xmlns="http://www.w3.org/2000/svg" onclick={if onclick.is_some() {onclick.clone().unwrap()} else {Callback::from(|_|{})}} {width} {height} viewBox={format!("0 0 {width} {height}")} fill={fill}
